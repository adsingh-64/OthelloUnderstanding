...
if __name__ == "__main__":
    ...
    setups = []  # list of tuples: (intervention_query, control_query, dt_queries)

    # RNG for randomized assignment of intervention/control
    rng = np.random.default_rng(0)

    # Define the edge squares adjacent to center
    edge_squares = []
    
    # A2-A5 (left edge, x=0, y=2..5)
    for y in range(2, 6):
        edge_squares.append((0, y, 'col'))  # column points to middle
    
    # H2-H5 (right edge, x=7, y=2..5)
    for y in range(2, 6):
        edge_squares.append((7, y, 'col'))  # column points to middle
    
    # C0-F0 (bottom edge, x=2..5, y=0)
    for x in range(2, 6):
        edge_squares.append((x, 0, 'row'))  # row points to middle
    
    # C7-F7 (top edge, x=2..5, y=7)
    for x in range(2, 6):
        edge_squares.append((x, 7, 'row'))  # row points to middle

    # Generate patterns for each edge square
    for x, y, axis_type in edge_squares:
        s0 = sq_name(x, y)
        
        # Determine directions toward center
        if x == 0:  # Left edge (A column)
            col_dir = (1, 0)  # move right toward center
            # Diagonal options: up-right or down-right
            if y < 3.5:
                diag_dir = (1, 1)  # up-right
            else:
                diag_dir = (1, -1)  # down-right
        elif x == 7:  # Right edge (H column)
            col_dir = (-1, 0)  # move left toward center
            # Diagonal options: up-left or down-left
            if y < 3.5:
                diag_dir = (-1, 1)  # up-left
            else:
                diag_dir = (-1, -1)  # down-left
        elif y == 0:  # Bottom edge (row 0)
            row_dir = (0, 1)  # move up toward center
            # Diagonal options: up-left or up-right
            if x < 3.5:
                diag_dir = (1, 1)  # up-right
            else:
                diag_dir = (-1, 1)  # up-left
        else:  # Top edge (row 7)
            row_dir = (0, -1)  # move down toward center
            # Diagonal options: down-left or down-right
            if x < 3.5:
                diag_dir = (1, -1)  # down-right
            else:
                diag_dir = (-1, -1)  # down-left
        
        # Build patterns based on axis type
        if axis_type == 'col':
            # Column pattern
            ax, ay = col_dir
        ...
        # Create queries
        axis_query = [
            Condition(feature_name=f'{s0} blank', operator='>', threshold=-1),
            Condition(feature_name=f'{ax_s1} mine-theirs', operator='<=', threshold=1),
            Condition(feature_name=f'{ax_s2} mine-theirs', operator='>', threshold=-1),
        ]
        
        diag_query = [
            Condition(feature_name=f'{s0} blank', operator='>', threshold=-1),
            Condition(feature_name=f'{dg_s1} mine-theirs', operator='<=', threshold=1),
            Condition(feature_name=f'{dg_s2} mine-theirs', operator='>', threshold=-1),
        ]
        
        # Randomize which is intervention vs control
        if rng.random() < 0.5:
        ...
        dt_queries = [dt_first_two]
        setups.append((intervention_query, control_query, dt_queries))

    # Run experiments across setups and aggregate
    ...
    for (intervention_query, control_query, dt_queries) in setups:
        ...
        # Skip if either side has no positions
        if len(intervention_positions_encoded) == 0 or len(control_positions_encoded) == 0:
            continue

        rprint(f"[dim]Setup with legal square {intervention_query[0].feature_name.split()[0]}: |interv|={len(intervention_positions_encoded)}, |ctrl|={len(control_positions_encoded)}[/dim]")
    ...
    rprint(f"\n[bold]Aggregated over {used} setups (edge squares only: A2-A5, H2-H5, C0-F0, C7-F7)[/bold]")

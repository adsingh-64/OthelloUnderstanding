...
CONT_DT VERSION - ORIGINAL 48 SQUARES SETUP
...
if __name__ == "__main__":
    ...
    # Helper utilities to generate multiple three-square patterns while avoiding center squares
    letters = 'ABCDEFGH'
    ...
    # Directions: HV and Diagonals
    HV_DIRS = [(1,0), (-1,0), (0,1), (0,-1)]
    DG_DIRS = [(1,1), (1,-1), (-1,1), (-1,-1)]
    ...
    # Iterate over all squares outside the 4x4 C2..F5 region
    for y in range(8):
        for x in range(8):
            if 2 <= x <= 5 and 2 <= y <= 5:
            ...
            # Choose a row/col direction that points to the middle (only one axis applies unless on a diagonal)
            if sx != 0 and sy == 0:
                axis_dir = (sx, 0)
            elif sx == 0 and sy != 0:
                axis_dir = (0, sy)
            else:
                # On a diagonal relative to center; choose row or column based on which is farther from mid
                axis_dir = (sx, 0) if abs(x - 3.5) >= abs(y - 3.5) else (0, sy)

            # Choose a diagonal direction pointing to the middle
            if sx != 0 and sy != 0:
                diag_dir = (sx, sy)
            elif sx != 0:
            ...
            # Ensure both directions can take two steps in-bounds
            (ax, ay) = axis_dir
            (dx, dy) = diag_dir
            if not (in_bounds(x + 2*ax, y + 2*ay) and in_bounds(x + 2*dx, y + 2*dy)):
                continue

            # Build the three-square patterns for axis and diagonal
            s0 = sq_name(x, y)
            ...
            # Randomize which is intervention vs control to avoid confounding
            if rng.random() < 0.5:
